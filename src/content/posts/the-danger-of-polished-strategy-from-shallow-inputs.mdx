...
## Fixed inputs often stop too early

Fixed inputs can help collect structured answers quickly.

They are weaker when the team needs to understand why the answer is true.

The problem is not that fixed inputs are bad. The problem is that many decisions require the context behind the field.
