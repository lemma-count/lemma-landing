...
## Fixed inputs capture the label

A fixed input is efficient because it constrains the customer.
...
That is the opening for adaptive voice conversations.
...
A fixed form is still the right tool when the answer is simple, factual, or administrative. Nobody needs an adaptive voice conversation to collect a shipping address.
...
Ask fewer generic questions. Ask better follow-ups. Preserve the respondent's context. Turn the result into usable context.
...
That is what fixed inputs miss.
