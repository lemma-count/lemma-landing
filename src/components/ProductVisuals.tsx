    ...
    >
      <div className="hidden border-r border-[#E1E7F0] bg-white p-3 sm:block lg:p-4">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-[#EEF3FF] px-2 py-1 text-[9px] font-semibold text-brand-cobalt">Prioritized</span>
        </div>
        <div className="mt-3 space-y-2">
          {prospects.map((prospect, index) => (
            <div
              key={prospect.name}
              className={`rounded-[9px] border p-2.5 ${index === 0 ? "border-brand-cobalt/28 bg-[#F7F9FF] shadow-[0_10px_24px_-20px_rgba(43,87,213,0.8)]" : "border-[#E7EAF0] bg-white"}`}
            >
              <div className="flex items-center gap-2.5">
                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[9px] font-semibold ${index === 0 ? "bg-brand-cobalt text-white" : "bg-[#EEF1F5] text-[#546175]"}`}>
                  {prospect.initials}
                </span>
                ...
                <span className="text-[9px] font-semibold text-brand-cobalt">{prospect.score}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="relative overflow-hidden p-3 sm:p-4 lg:p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[9px] font-semibold uppercase tracking-[0.13em] text-brand-cobalt">Alex Morgan · Northstar</p>
        ...
        <div className="relative mt-4 pl-5 sm:mt-5 sm:pl-6">
          <span className="absolute bottom-3 left-[7px] top-3 w-px bg-gradient-to-b from-brand-cobalt via-brand-cobalt/45 to-[#63A878] sm:left-[9px]" aria-hidden />
...
function JourneyRow({
  ...
  return (
    <div className="relative mb-3 last:mb-0 sm:mb-4">
      <span className={`absolute -left-5 top-0.5 flex h-4 w-4 items-center justify-center rounded-full sm:-left-6 sm:h-5 sm:w-5 ${complete ? "bg-[#58A96F] text-white" : "bg-brand-cobalt text-white"}`} aria-hidden>
        {icon}
      </span>
      ...
      <div className="mt-3 space-y-2">
        {["Strong match · active trigger", "Strong match · shared context", "Good match · research next"].map((label, index) => (
          <div key={label} className="flex items-center gap-2 rounded-lg border border-[#E7EAF0] bg-white p-2">
            ...
            <span className="rounded bg-[#EEF3FF] px-1.5 py-0.5 text-[8px] font-semibold text-brand-cobalt">{["92", "87", "81"][index]}</span>
          </div>
        ))}
      </div>
      <div className="mt-3 flex items-center gap-1.5 rounded-lg bg-brand-night px-2.5 py-2 text-[9px] font-medium text-white">
        <SparkleIcon size={11} weight="fill" className="text-brand-sky" aria-hidden /> Start with the 34 strongest opportunities
      </div>
    </VisualFrame>
  );
}
...
export function AdaptiveConversationVisual() {
  return (
    <VisualFrame label="Live prospect reasoning" icon={<LightningIcon size={13} weight="fill" />}>
      ...
      <div className="my-2 flex items-center gap-2 pl-2 text-[8px] font-semibold uppercase tracking-[0.12em] text-brand-cobalt">
        <span className="h-px flex-1 bg-brand-cobalt/24" /> Lemma decides <ArrowRightIcon size={10} weight="bold" />
      </div>
      <div className="rounded-lg border border-brand-cobalt/22 bg-[#F3F6FF] p-2.5">
        <p className="text-[8px] font-semibold uppercase tracking-[0.12em] text-brand-cobalt">Next action</p>
...
export function HandoverVisual() {
  return (
    <VisualFrame label="Qualified handover" icon={<UserCircleCheckIcon size={13} weight="fill" />}>
      ...
      <p className="mt-3 flex items-center gap-1.5 text-[9px] font-semibold text-brand-cobalt">
  ...
  return (
    <div role="img" aria-label={`Illustrative Lemma product view: ${label}.`} className="h-full rounded-[8px] bg-[#F6F8FC] p-3 sm:p-4">
      <div className="flex items-center gap-2 border-b border-[#E1E7F0] pb-2.5 text-[10px] font-semibold text-ink">
        <span className="flex h-6 w-6 items-center justify-center rounded-md bg-[#EAF0FF] text-brand-cobalt" aria-hidden>{icon}</span>
        {label}
      </div>
  ...
  return (
    <div role="img" aria-label="Illustration of Lemma autonomy settings, approval rules, activity history, and pause control." className="overflow-hidden rounded-[10px] border border-[#E2E7EF] bg-[#F6F8FC]">
      <div className="flex items-center justify-between border-b border-[#E1E7F0] bg-white px-4 py-3">
        <span className="flex items-center gap-2 text-[11px] font-semibold text-ink"><SlidersHorizontalIcon size={14} weight="bold" className="text-brand-cobalt" /> Operating rules</span>
        ...
        <div className="space-y-2">
          {controls.map((control) => (
            <div key={control.label} className="flex items-center gap-2.5 rounded-lg border border-[#E2E7EF] bg-white p-2.5">
              <span className="flex h-7 w-7 items-center justify-center rounded-md bg-[#EEF3FF] text-brand-cobalt">{control.icon}</span>
              <span className="min-w-0 flex-1 text-[10px] font-semibold text-ink">{control.label}</span>
              <span className={`relative h-4 w-7 rounded-full ${control.accent ? "bg-brand-cobalt" : "bg-[#CDD4DF]"}`}>
...
function ActivityLine({ title, time, complete = false }: { title: string; time: string; complete?: boolean }) {
  return (
    <div className="flex gap-2">
      <span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full ${complete ? "bg-[#EAF7EE] text-[#347A4A]" : "bg-[#EEF3FF] text-brand-cobalt"}`}>
